...
const ShopPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get("https://artistry-backend-ss49.onrender.com/products");
  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      try {
        const response =await axios.get("https://artistry-backend-ss49.onrender.com/materials");
  ...
  useEffect(() => {
    const fetchProductRatings = async (productId) => {
      try {
        const response = await axios.get(`https://artistry-backend-ss49.onrender.com/ratings/product/${productId}`);
  ...
  useEffect(() => {
    const fetchMaterialRatings = async (materialId) => {
      try {
        const response = await axios.get(`https://artistry-backend-ss49.onrender.com/ratings/material/${materialId}`);
  ...
  const addToCart = (id, name) => {
   ...
   axios.post("https://artistry-backend-ss49.onrender.com/addcart", { productId: id }, { headers: { userId: decode.id } })
      .then(() => {
  ...
  const addMaterialToCart = (id, name) => {
    ...
    const decode = jwtDecode(token);
    axios.post("https://artistry-backend-ss49.onrender.com/addcartmeterial", { materialId: id }, { headers: { userId: decode.id } })
      .then(() => {
        alert(`${name} added successfully!`);
      })
      .catch((err) => {
