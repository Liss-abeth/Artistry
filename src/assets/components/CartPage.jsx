...
const CartPage = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      try {
        ...
        const res = await axios.get("https://artistry-backend-ss49.onrender.com/getcart", {
  ...
  const handleQuantityChange = async (item, delta) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const decode = jwtDecode(token);
      await axios.put("https://artistry-backend-ss49.onrender.com/updatecart", {
        userId: decode.id,
  ...
  const handleRemoveItem = async (item) => {
    try {
      const token = localStorage.getItem("token");
      const decode = jwtDecode(token);
      await axios.delete("https://artistry-backend-ss49.onrender.com/removecartitem", {
        data: {
          userId: decode.id,
          productId: item.productId?._id || null,
          materialId: item.materialId?._id || null,
