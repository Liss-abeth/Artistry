...
const OrderDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const { data } = await axios.get(`https://artistry-backend-ss49.onrender.com/orders/user/${userId}`);
        setOrders(data);

        const ratingsResponse = await axios.get(`https://artistry-backend-ss49.onrender.com/ratings/user/${userId}`);
  ...
  const handleRatingChange = async (orderId, item, rating) => {
    ...
    try {
      await axios.post(`https://artistry-backend-ss49.onrender.com/rate/${itemType}/${itemId}`, payload, {
