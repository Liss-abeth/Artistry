...
console.log("✅ Product IDs saved:", artistProducts.map(p => p._id));
  ...
  const fetchProducts = async (artistId) => {
    try {
      const response = await axios.get(`http://localhost:1011/products/artist/${artistId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editProductId) {
        await axios.put(`http://localhost:1011/updateproduct/${editProductId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        alert("Product updated successfully!");
        setEditProductId(null);
      } else {
        await axios.post("http://localhost:1011/addproduct", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:1011/deleteproduct/${id}`);
  ...
  const fetchArtistData = async () => {
    ...
    try {
        ...
        const { data } = await axios.get(`http://localhost:1011/artists/${artistId}`);  // Ensure correct API route
...
useEffect(() => {
    ...
    const fetchEarnings = async () => {
      try {
        const response = await axios.get(`/earnings/${artistId}`); 
    ...
    const fetchNotifications = async () => {
      try {
        const response = await axios.get(`/notifications/${artistId}`); 
    ...
    const updateOrderStatus = async (orderId, status) => {
      try {
        await axios.put(`http://localhost:1011/orders/${orderId}`, { status });
        fetchOrders();
      } catch (error) {
  ...
  const fetchUser = async () => {
    ...
    try {
      const res = await fetch(`/api/user/${userId}`); // Adjust endpoint if needed
  ...
  const fetchUserName = async (userId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:1011/users/${userId}`);
  ...
  const fetchProductName = async (productId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:1011/products/${productId}`);
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(`http://localhost:1011/orders/artist/${artistId}`);
  ...
  const handleViewOrders = () => {
    ...
    if (!showOrders) {
      const fetchOrders = async () => {
        try {
          const response = await fetch("http://localhost:1011/getorders");
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.put("http://localhost:1011/updateorder", { orderId, status: newStatus });
